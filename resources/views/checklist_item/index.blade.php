...
<div class="dashboard">
    ...
    <main class="main-content">
        <div class="page-header">
            <h1><i class="fas fa-clipboard-list"></i> Checklist Item</h1>
            <a href="{{ route('checklist-items.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i> Tambah</a>
        </div>
        @if (session('success'))
            ...
            <div class="card-body">
                @if ($data->isEmpty())
                    <div class="empty-state">
                        ...
                        <h3>Belum ada item checklist</h3>
                        <p>Isi dengan data demo atau input manual.</p>
                        <div class="empty-actions">
                            <form method="POST" action="{{ route('checklist-items.demo') }}" style="display:inline;">
                                @csrf <input type="hidden" name="count" value="5">
                                <button class="btn btn-success"><i class="fas fa-database"></i> Demo 5</button>
                            </form>
                            <form method="POST" action="{{ route('checklist-items.demo') }}" style="display:inline;">
                                @csrf <input type="hidden" name="count" value="10">
                                <button class="btn btn-success"><i class="fas fa-database"></i> Demo 10</button>
                            </form>
                            <form method="POST" action="{{ route('checklist-items.demo') }}" style="display:inline;">
                                @csrf <input type="hidden" name="count" value="15">
                                <button class="btn btn-success"><i class="fas fa-database"></i> Demo 15</button>
                            </form>
                        </div>
                        <a href="{{ route('checklist-items.create') }}" class="btn btn-outline"><i class="fas fa-plus"></i> Input Manual</a>
                    </div>
                @else
                    <table>
                        <thead><tr><th>No</th><th>Nama Item</th><th>Deskripsi</th><th>Status</th><th>Aksi</th></tr></thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="actions">
                                        <a href="{{ route('checklist-items.edit', $item) }}" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i></a>
                                        <form method="POST" action="{{ route('checklist-items.destroy', $item) }}" onsubmit="confirmForm(this, 'Hapus data ini?')">@csrf @method('DELETE')<button class="btn btn-danger btn-sm"><i class="fas fa-trash"></i></button></form>
                                    </div>
                                </td>
                            </tr>
                            @endforeach
