...
<div class="dashboard">
    ...
    <main class="main-content">
        <div class="page-header">
            <h1><i class="fas fa-ruler"></i> {{ isset($unit) ? 'Edit Satuan' : 'Tambah Satuan' }}</h1>
        </div>
            ...
            <div class="card-body">
                <form method="POST" action="{{ isset($unit) ? route('units.update', $unit) : route('units.store') }}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="symbol" name="symbol" class="form-control" value="{{ old('symbol', $unit->symbol ?? '') }}" placeholder="Contoh: pcs, L, kg">
                        @error('symbol') <small style="color:#dc2626;">{{ $message }}</small> @enderror
                    </div>
