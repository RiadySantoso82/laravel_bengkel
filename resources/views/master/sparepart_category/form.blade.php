...
@section('title', isset($sparepartCategory) ? 'Edit Kategori Sparepart' : 'Tambah Kategori Sparepart')
...
<div class="dashboard">
    ...
    <main class="main-content">
        <div class="page-header">
            <h1><i class="fas fa-tags"></i> {{ isset($sparepartCategory) ? 'Edit Kategori Sparepart' : 'Tambah Kategori Sparepart' }}</h1>
        </div>

        <div class="card">
            <div class="card-body">
                <form method="POST" action="{{ isset($sparepartCategory) ? route('sparepart-categories.update', $sparepartCategory) : route('sparepart-categories.store') }}">
                    @csrf
                    @if (isset($sparepartCategory)) @method('PUT') @endif

                    <div class="form-group">
                        <label for="name">Nama Kategori</label>
                        <input type="text" id="name" name="name" class="form-control" value="{{ old('name', $sparepartCategory->name ?? '') }}" required>
                        @error('name') <small style="color:#dc2626;">{{ $message }}</small> @enderror
                    </div>

                    <div class="form-group">
                        <label for="description">Deskripsi</label>
                        <textarea id="description" name="description" class="form-control">{{ old('description', $sparepartCategory->description ?? '') }}</textarea>
                    </div>

                    <div class="form-actions">
                        <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Simpan</button>
                        <a href="{{ route('sparepart-categories.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Kembali</a>
                    </div>
                </form>
            </div>
        </div>
