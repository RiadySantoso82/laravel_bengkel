...
@section('title', isset($serviceCategory) ? 'Edit Kategori Servis' : 'Tambah Kategori Servis')
...
<div class="dashboard">
    ...
    <main class="main-content">
        <div class="page-header">
            <h1><i class="fas fa-toolbox"></i> {{ isset($serviceCategory) ? 'Edit Kategori Servis' : 'Tambah Kategori Servis' }}</h1>
        </div>

        <div class="card">
            <div class="card-body">
                <form method="POST" action="{{ isset($serviceCategory) ? route('service-categories.update', $serviceCategory) : route('service-categories.store') }}">
                    @csrf
                    @if (isset($serviceCategory)) @method('PUT') @endif

                    <div class="form-group">
                        <label for="name">Nama Kategori</label>
                        <input type="text" id="name" name="name" class="form-control" value="{{ old('name', $serviceCategory->name ?? '') }}" required>
                        @error('name') <small style="color:#dc2626;">{{ $message }}</small> @enderror
                    </div>

                    <div class="form-group">
                        <label for="description">Deskripsi</label>
                        <textarea id="description" name="description" class="form-control">{{ old('description', $serviceCategory->description ?? '') }}</textarea>
                    </div>

                    <div class="form-actions">
                        <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Simpan</button>
                        <a href="{{ route('service-categories.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Kembali</a>
                    </div>
                </form>
            </div>
        </div>
